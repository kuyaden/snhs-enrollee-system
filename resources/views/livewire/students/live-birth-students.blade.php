<div class="p-6 space-y-6">
    ...
    <div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/10 dark:to-emerald-900/10 p-6 rounded-2xl border border-green-100 dark:border-green-800">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Live Birth Certificate Records</h2>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Manage and track student birth certificate submissions</p>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <label class="inline-flex items-center gap-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800 cursor-pointer hover:bg-green-100 dark:hover:bg-green-900/30 transition-all duration-200">
                <input type="checkbox"
                       wire:model.live="onlyWithLiveBirth"
                       class="w-4 h-4 rounded border-gray-300 text-green-600 shadow-sm focus:ring-green-500 dark:bg-gray-800 dark:border-gray-600 dark:focus:ring-green-400 transition-all duration-200" />
                <div class="flex items-center gap-2">
                    <svg class="w-5 h-5 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    <span class="text-sm font-medium text-green-700 dark:text-green-300">Show only with Live Birth</span>
                </div>
            </label>
        </div>
    </div>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 border-b border-gray-200 dark:border-gray-600">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left font-semibold text-gray-700 dark:text-gray-300 text-xs uppercase tracking-wider">Documents</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($students as $student)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-200 group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-3">
                                        <div class="relative">
                                            ...
                                            @else
                                                <div class="w-10 h-10 bg-gradient-to-br from-green-400 to-green-600 rounded-xl flex items-center justify-center shadow-sm">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-900 dark:text-white group-hover:text-green-600 dark:group-hover:text-green-400 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    @if ($student->live_birth || $student->live_birth_one || $student->live_birth_two)
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $docs = [
                                            'Main Document' => $student->live_birth,
                                            'Page 1' => $student->live_birth_one,
                                            'Page 2' => $student->live_birth_two,
                                        ];
                                        $hasDocuments = $student->live_birth || $student->live_birth_one || $student->live_birth_two;
                                    @endphp
                                            ...
                                            @foreach ($docs as $label => $file)
                                                @if ($file)
                                                    <div class="flex items-center gap-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-2 shadow-sm">
                                                        ...
                                                        <span class="text-xs text-gray-500 dark:text-gray-400 ml-1">({{ $label }})</span>
                                                    </div>
                                                @endif
                                            @endforeach
                                        </div>
                                    @else
                                        <span class="text-gray-400 dark:text-gray-500 text-sm italic">No documents uploaded</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
